<?php
//军团
class CrossPlayerArmy extends CrossModelBase{
	...
	public function addByData($playerId, $data){
		...
		$CrossPlayerGeneral->battleId = $this->battleId;
		foreach($data as $_i => $_d){
			$CrossPlayerArmyUnit->add($playerId, $armyId, $_i+1, $_d[0], $_d[1], $_d[2]);
			$_playerGeneral = $CrossPlayerGeneral->getByGeneralId($playerId, $_d[0]);
			$CrossPlayerGeneral->assign($_playerGeneral)->updateArmy($armyId);
		}
		return true;
	}
