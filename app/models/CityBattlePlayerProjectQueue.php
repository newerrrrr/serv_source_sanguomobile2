<?php
class CityBattlePlayerProjectQueue extends CityBattleModelBase{
    public $blacklist = [];
    public $battleId;
	...
	public function hasActivityQueue($playerId){
        $ret = self::find("player_id={$playerId} and battle_id={$this->battleId} and status=1")->toArray();
        if(!empty($ret)){
            return true;
        }else{
     ...
     */
    public function addQueue($playerId, $campId, $targetPlayerId, $type, $needTime, $armyId=0, $targetInfo=[], $extraData=[]){
		$accelerate_info = [];
	...
	public static function calculateMoveTime($battleId, $playerId, $fromX, $fromY, $toX, $toY, $type, $armyId=0, $debuff=0, $buff=0){
		...
		*/
		...
		/*if($type == 4){
			...
			if($map && $map['map_element_origin_id'] == 406){//判断是否是玩家城堡
				//判断目标玩家buff
		...
		*/
		...
		$speedBuff += $CityBattleCamp->getByPlayerId($playerId)['buff_move'];
	...
	public function callbackQueue($id, $toX, $toY, $option=[]){
		...
		if(!@$inDispWorker){
			while(!$DispatcherTask->cacheAddXY(Cache::db('dispatcher', 'CityBattle'), $this->battleId, $toX, $toY)){
				sleep($perTry);
				$i++;
				if($i >= $tryLimit){
					return false;
		...
		if(!$ppq || $ppq->status != 1){
			if(!@$inDispWorker)
				$DispatcherTask->cacheRemoveXY(Cache::db('dispatcher', 'CityBattle'), $this->battleId, $ppq->to_x, $ppq->to_y);
			return false;
		}
		...
		if($ppq->type == self::TYPE_LADDER_ING && @$option['ladder']){
			...
			//更新英勇值
			$now = time();
			$addScore = max(0, floor(($now - strtotime($ppq->create_time)) * (new CountryBasicSetting)->getValueByKey('get_ladder_score') / 100));
			if($addScore){
				$Player->alter($ppq->player_id, ['score'=>'score+'.$addScore]);
				(new CityBattleCommonLog)->add($ppq->battle_id, $ppq->player_id, $ppq->camp_id, '更新英勇值+'.$addScore.'|by占领云梯.');
			}
			...
			if(@$option['rightnow']){
				...
				$needTime = self::calculateMoveTime($ppq->battle_id, $ppq->player_id, $extraData['from_x'], $extraData['from_y'], $extraData['to_x'], $extraData['to_y'], 3, $ppq->army_id);
			}
			$newQueueId = $this->addQueue($ppq->player_id, $ppq->camp_id, 0, $returnType, $needTime, $ppq->army_id, $targetInfo, $extraData);
			if(!$newQueueId){
				if(!@$inDispWorker)
					$DispatcherTask->cacheRemoveXY(Cache::db('dispatcher', 'CityBattle'), $this->battleId, $ppq->to_x, $ppq->to_y);
				return false;
			...
			$ppqs = self::find($condition)->toArray();
			...
			//新增回家队列
			$newQueueId = $this->addQueue($ppq->player_id, $ppq->camp_id, 0, $returnType, ['create_time'=>$createTime, 'end_time'=>$endTime], $ppq->army_id, $targetInfo, $extraData);
			if(!$newQueueId){
				if(!@$inDispWorker)
					$DispatcherTask->cacheRemoveXY(Cache::db('dispatcher', 'CityBattle'), $this->battleId, $ppq->to_x, $ppq->to_y);
				return false;
			...
			if(in_array($ppq->type, [self::TYPE_CITYBATTLE_GOTO])){
				crossSocketSend(CrossPlayer::parsePlayerId($ppq->target_player_id)['server_id'], ['Type'=>'citybattle_cancelattacked', 'Data'=>['playerId'=>[$ppq->target_player_id]]]);//todo
			}
	...
	public function callbackPlayerQueue($battleId, $playerId){
		...
		foreach($ppqs as $_ppq){
			if(in_array($_ppq->type, array_merge($this->moveTypes, $this->stayTypes))){//如果是回城队列，修改结束时间
				...
				(new CityBattleCommonLog)->add($battleId, $_ppq->player_id, $_ppq->camp_id, '部队遣返[queueId='.$_ppq->id.']');
			}
		}
		return true;
	}

	public function callbackCatapult($battleId, $area, $campId=[], $stopIfFail=false, &$playerIds=[]){
		$playerIds = [];
		if(!is_array($area) || isset($area['area'])){
			$condition = ['type='.self::TYPE_CATAPULT_ING.' and battle_id='.$battleId.' and area='.$area.' and status=1'.($campId ? ' and camp_id in ('.join(',', $campId).')' : '')];
		}elseif(is_array($area) && isset($area['section'])){
			$Map = new CityBattleMap;
			$mapIds = Set::extract("/id", array_values($Map->getAllBySection($battleId, $area['section'])));
			if(!$mapIds)
				return true;
			$condition = ['type='.self::TYPE_CATAPULT_ING.' and battle_id='.$battleId.' and  status=1 and to_map_id in ('.join(',', $mapIds).')'.($campId ? ' and camp_id in ('.join(',', $campId).')' : '')];
		}else{
			return false;
		}
		$ppqs = self::find($condition);
		foreach($ppqs as $_ppq){
			...
			(new CityBattleCommonLog)->add($battleId, $_ppq->player_id, $_ppq->camp_id, '投石车部队遣返[queueId='.$_ppq->id.']');
	...
	public function callbackCrossbow($battleId, $area, $stopIfFail=false, &$playerIds=[]){
		...
		if(!is_array($area) || isset($area['area'])){
			...
			if(!$mapIds)
				return true;
			$condition = ['type='.self::TYPE_CROSSBOW_ING.' and battle_id='.$battleId.' and  to_map_id in ('.join(',', $mapIds).') and status=1'];
		}else{
			return false;
		}
		$ppqs = self::find($condition);
		foreach($ppqs as $_ppq){
			...
			(new CityBattleCommonLog)->add($battleId, $_ppq->player_id, $_ppq->camp_id, '床弩部队遣返[queueId='.$_ppq->id.']');
			$playerIds[] = $_ppq->player_id;
		}
		$playerIds = array_unique($playerIds);
		return true;
	...
	public function callbackHammer($battleId, $area, $mapId=0){
		...
		foreach($ppqs as $_ppq){
			...
			(new CityBattleCommonLog)->add($battleId, $_ppq->player_id, $_ppq->camp_id, '攻城锤部队遣返[queueId='.$_ppq->id.']');
		}
		return true;
	}
	
	public function callbackLadder($battleId, $area, $mapId=0){
		if(!is_array($area) || isset($area['area'])){
			$condition = ['type='.self::TYPE_LADDER_ING.' and battle_id='.$battleId.' and area='.$area.($mapId ? ' and to_map_id='.$mapId : '').' and status=1'];
		}elseif(is_array($area) && isset($area['section'])){
			$Map = new CityBattleMap;
			$mapIds = Set::extract("/id", array_values($Map->getAllBySection($battleId, $area['section'])));
			if(!$mapIds)
		...
		foreach($ppqs as $_ppq){
			$this->callbackQueue($_ppq->id, $_ppq->to_x, $_ppq->to_y);
			(new CityBattleCommonLog)->add($battleId, $_ppq->player_id, $_ppq->camp_id, '云梯部队遣返[queueId='.$_ppq->id.']');
		}
		return true;
	}
