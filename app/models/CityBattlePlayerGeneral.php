<?php
class CityBattlePlayerGeneral extends CityBattleModelBase{
	...
	public function add($data){
		...
		$serverId = CityBattlePlayer::parsePlayerId($data['player_id'])['server_id'];
...
//$bs['value_formula'] = $bs['value_formula_2'] = '500 + $force * 1 + $skill_lv * 25 + floor($skill_lv/10) * 250';
	...
	public function getMaxBringSoldier(){
		...
		$CityBattlePlayer->battleId = $this->battle_id;
		$player = $CityBattlePlayer->getByPlayerId($this->player_id);
	...
	public function getTotalAttrCacheKey($generalId){
		return 'CityBattlePlayerGeneralAttr:'.$generalId;
	}
	...
	/*public function getFitableSoldier($playerId, $generalId){
		...
		return ['soldier_id'=>(new CityBattleController)->soldierTypeIds[$soldierType][0], 'num'=>min($bringSoldierMax, $num)];
	}
	*/
     ...
     * @return <type>
     */
	public function getMaxAttrByCamp($campId, $attr, $generalId=0){
		$generals = $this->sqlGet('select player_id, general_id, total_attr from '.$this->getSource().' where battle_id='.$this->battleId.' and player_id in (select player_id from '.(new CityBattlePlayer)->getSource().' where battle_id='.$this->battleId.' and camp_id='.$campId.')'.($generalId ? ' and general_id='.$generalId : '').' order by star_lv desc');
		$maxAttr = 0;
		$maxPlayerId = 0;
		$maxGeneralId = 0;
	...
	public function getMaxAttrsByCamps($campIds, $generalId=0){
		$generals = $this->sqlGet('select player_id, general_id, total_attr from '.$this->getSource().' where battle_id='.$this->battleId.' and player_id in (select player_id from '.(new CityBattlePlayer)->getSource().' where battle_id='.$this->battleId.' and camp_id in ('.join(',', $campIds).'))'.($generalId ? ' and general_id='.$generalId : '').' order by star_lv desc');
		$maxAttr = 0;
		$maxGeneralId = 0;
		$ret = [];
		foreach($generals as $_g){
			$vv = json_decode($_g['total_attr'], true)['attr'];
			foreach($vv as $k => $v){
				if($v > @$ret[$k]){
					$ret[$k] = $v;
				}
