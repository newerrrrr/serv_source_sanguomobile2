<?php
//军团配置
class CityBattlePlayerArmyUnit extends CityBattleModelBase{
     ...
     * @return <type>
     */
	public function add($playerId, $armyId, $unit, $generalId, $soldierId, $soldierNum, $maxSoldierNum=0){
		if(!$soldierNum)
			$soldierId = 0;
	...
	public function updateSoldier($playerId, $generalId, $soldierId){
		...
		$ret = $this->updateAll(array(
			...
			'update_time'=>"'".$now."'",
			'rowversion'=>"'".uniqid()."'",
		), ['battle_id'=>$this->battleId, "player_id"=>$playerId, 'general_id'=>$generalId, 'soldier_num >'=>0]);
		$this->clearDataCache($playerId);
		//(new Player)->refreshPower($playerId, 'army_power');
		return $ret;
	}
	...
	public function _clearDataCache($playerId=0){
		...
		$CityBattlePlayerArmy->battleId = $this->battleId;
		$CityBattlePlayerArmy->clearDataCache($playerId);
	}
	...
	public function fullfill($playerId, $armyId=0){
		$now = date('Y-m-d H:i:s');
		$CityBattlePlayerArmy = new CityBattlePlayerArmy;
		//$CityBattlePlayerArmy->battleId = $this->battleId;
		if($armyId){
			...
			$armyIds = Set::extract('/id', $CityBattlePlayerArmy->find(['battle_id='.$this->battleId.' and player_id='.$playerId.' and status=0'])->toArray());
		}
		/*$armies = $this->find(['battle_id='.$this->battleId." and player_id=".$playerId.' and army_id in ('.join(',', $armyIds).')'])->toArray();
		$flag = false;
		foreach($armies as $_army){
			if($_army['soldier_num'] < $_army['max_soldier_num']){
				$flag = true;
				break;
			}
		}
		...
		*/
		$ret = $this->updateAll(array(
			'soldier_id' => 'if(soldier_id, soldier_id, last_soldier_id)',
			'soldier_num' => 'max_soldier_num',
			'update_time'=>"'".$now."'",
			'rowversion'=>"'".uniqid()."'",
		), ['battle_id'=>$this->battleId, "player_id"=>$playerId, 'army_id'=>$armyIds, 'soldier_num <'=>'max_soldier_num']);
		if($ret)
			$this->clearDataCache($playerId);
		return $ret;
	}
	
	public function getGroupNumByType($playerId, $armyId, $soldierType){
		$ret = $this->getByArmyId($playerId, $armyId);
		if(!$ret)
			return 0;
		$num = 0;
		foreach($ret as $_r){
			if(!$_r['soldier_id']) continue;
			if(substr($_r['soldier_id'], 0, 1) == $soldierType){
				$num++;
			}
		}
		return $num;
     ...
     * @return <type>
     */
	public function getHomeSoldier($playerId){
		$ret = $this->sqlGet('select sum(soldier_num) from '.$this->getSource().' where army_id in (select id from city_battle_player_army where player_id='.$playerId.' and status=0)');
		if(!$ret)
			return 0;
		return $ret[0]['sum(soldier_num)'];
	}
